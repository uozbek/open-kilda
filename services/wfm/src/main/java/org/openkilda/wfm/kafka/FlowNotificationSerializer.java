...
@Slf4j
public class FlowNotificationSerializer implements Serializer<FlowNotification> {
    @Override
    ...
    @Override
    public byte[] serialize(String topic, FlowNotification data) {
        if (data == null) {
            return null;
        }

        try {
            return SerializationUtils.MAPPER.writeValueAsBytes(data);
        } catch (IOException e) {
