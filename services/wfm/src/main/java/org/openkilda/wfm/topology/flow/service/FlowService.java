        ...
        ImmutablePair<UpdatedFlow, UpdatedFlow> result = transactionManager.doInTransaction(() -> {
            ...
            if (pathIds.isEmpty() || pathIds.contains(flow.getForwardPathId())
                ...
                boolean isFoundNewPath =
                        !buildFlowPath(flowId, pathPair.getForward()).equals(flow.getForwardPath())
                                || !buildFlowPath(flowId, pathPair.getReverse()).equals(flow.getReversePath());
                ...
                FlowPath newForwardPath = buildFlowPath(flow.getFlowId(), pathPair.getForward());
                FlowPath newReversePath = buildFlowPath(flow.getFlowId(), pathPair.getReverse());
            ...
            if (flow.isAllocateProtectedPath() && (pathIds.isEmpty()
                ...
                boolean isFoundNewPath =
                        !buildFlowPath(flowId, pathPair.getForward()).equals(flow.getProtectedForwardPath())
                                || !buildFlowPath(flowId, pathPair.getReverse()).equals(flow.getProtectedReversePath());
