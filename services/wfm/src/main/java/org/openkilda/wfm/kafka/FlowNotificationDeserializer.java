...
@Slf4j
public class FlowNotificationDeserializer implements SerializableDeserializer<FlowNotification> {
    @Override
    ...
    @Override
    public FlowNotification deserialize(String topic, byte[] data) {
        try {
            return SerializationUtils.MAPPER.readValue(data, FlowNotification.class);
        } catch (IOException e) {
