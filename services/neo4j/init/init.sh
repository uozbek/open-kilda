...
PATH=/opt/neo4j-community-3.3.9/bin:$PATH
...
if [ "${TEST_MODE}" == "true" ]; then
    ...
    if [ "${NEO4J_AUTH:-}" == "none" ]; then
        ...
        /opt/neo4j-community-3.3.9/bin/neo4j-admin set-initial-password "${password}" || true
    ...
    for i in $( set | grep ^NEO4J_ | awk -F'=' '{print $1}' | sort -rn ); do
        ...
        if [[ -n ${value} ]]; then
            if [[ ! "${setting}" =~ ^[0-9]+.*$ ]]; then
                if grep -q -F "${setting}=" /opt/neo4j-community-3.3.9/conf//neo4j.conf; then
                    # Remove any lines containing the setting already
                    sed --in-place "/${setting}=.*/d" /opt/neo4j-community-3.3.9/conf//neo4j.conf
                fi
                # Then always append setting to file
                echo "${setting}=${value}" >> /opt/neo4j-community-3.3.9/conf//neo4j.conf
            else
